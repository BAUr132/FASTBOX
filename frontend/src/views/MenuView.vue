...
const fetchMenu = async () => {
  ...
  loading.value = true;
  try {
    const response = await api.get(`/shops/${props.shop.id}/menu`);
    // На бэкенде приходит объект магазина с вложенными продуктами
    menuItems.value = response.data.products || [];
  ...
  <div id="screen-menu" class="screen screen-fade-in bg-gray-50 dark:bg-gray-900 min-h-screen pb-24">
    ...
    <div class="p-4 space-y-4">
      <div v-if="loading" class="space-y-4">
         <div v-for="n in 4" :key="n" class="bg-white dark:bg-gray-800 p-4 rounded-2xl flex gap-4 animate-pulse">
      ...
      <div v-else v-for="item in menuItems" :key="item.id" class="bg-white dark:bg-gray-800 p-4 rounded-2xl flex gap-4 border border-gray-100 dark:border-gray-700 shadow-sm">
        ...
        <div class="flex-grow">
          ...
          <div class="flex justify-between items-center mt-3">
            <span class="font-bold text-blue-600 dark:text-blue-400">{{ Math.round(item.price) }} ₸</span>
    ...
    <!-- Floating Cart Button (Desktop/Web fallback) -->
    <div v-if="cartStore.cartCount > 0" @click="emit('navigate', 'cart')" class="fixed bottom-6 inset-x-6 z-50 cursor-pointer active:scale-95 transition-transform md:hidden">
      <div class="bg-blue-600 dark:bg-blue-700 text-white rounded-2xl p-4 shadow-2xl flex justify-between items-center border border-blue-500/30 backdrop-blur-md">
